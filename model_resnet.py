...
class ResNet(Module):
    ...
    def stage1_forward(self, x): # [2048, 4, 4]
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
...
class MaskNet(nn.Module):
    ...
    def __init__(self): #[2*image(3*112*112), 2*mask(1*112*112)] = [8*112*112]->[2048*4*4]
        super(MaskNet, self).__init__()
        self.conv1 = Conv2d(8, 32, kernel_size = 7, stride = 2, padding = 3, bias = False) # 32 * 64 * 64
        self.bn1 = BatchNorm2d(32) 
        self.relu = ReLU(inplace = True) 
        self.maxpool = MaxPool2d(kernel_size = 3, stride = 2, padding = 1) # 32 * 32 * 32
        self.conv2 = Conv2d(in_channels = 32, out_channels = 128, kernel_size=3, stride=2, padding=1) # 128 * 16 * 16
        self.bn2 = BatchNorm2d(128)
        self.conv3 = Conv2d(in_channels = 128, out_channels = 512, kernel_size=3, stride=2, padding=1) # 512 * 8 * 8
        self.bn3 = BatchNorm2d(512)
        self.conv4 = Conv2d(in_channels = 512, out_channels = 2048, kernel_size=3, stride=2, padding=1) # 2048 * 4 * 4
