...
if __name__ == '__main__':
    ...
    os.chdir(cwd)
    landmark_dir = '/data/CelebA/list_landmarks_celeba.txt'

    with open(landmark_dir, 'r') as f:
        List = f.readlines()[2:]
        box_list = [i.strip().split() for i in List]
        box_name = [i[0] for i in box_list]
        box_list = [ [ [ i[2*j+1], i[2*j+2] ]for j in range(5) ] for i in box_list]
    ...
    for subfolder in tqdm(os.listdir(source_root)):
        ...
        for image_name in os.listdir(os.path.join(source_root, subfolder)):
            ...
            img_index = int(image_name.split('.')[0]) - 1
            '''
            ...
            if len(landmarks) == 0: # If the landmarks cannot be detected, the img will be discarded
                ...
                continue
            '''
            facial5points = box_list[img_index]
